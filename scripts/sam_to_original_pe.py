        ...
        with open(sam_with_original_reads_rev, 'wt') as fout_rev:
            for line in fin:
                if line[0] != '@':
                    ...
                    if len(bit_flag) >= 7 and bit_flag[-5] == '1' and bit_flag[-7] == '1' and mapq >= 10:
                    ...
                    elif len(bit_flag) >= 8 and bit_flag[-5] == '1' and bit_flag[-8] == '1' and mapq >= 10:
                        sam_seq = str(Seq(sam_seq).reverse_complement())
                        # trim reads by cigar hard clipping 
                        if cigar[-1] == 'H':
                            hc_right = int(re.split("([A-Z])",cigar)[-3])
                    ...
                    elif len(bit_flag) >= 7 and bit_flag[-6] == '1' and bit_flag[-7] == '1' and mapq >= 10:
                        # trim reads by cigar hard clipping 
                        if cigar[-1] == 'H':
                            hc_right = int(re.split("([A-Z])",cigar)[-3])
                            sam_seq = sam_seq[0 : -hc_right]
                        if 'H' in cigar[0:-1]:
                            ...
                            sam_seq = sam_seq[hc_left:]
                        fout_fwd.write(line.replace(line.split('\t')[9], sam_seq))
                    elif len(bit_flag) >= 8 and bit_flag[-6] == '1' and bit_flag[-8] == '1' and mapq >= 10:
                        # trim reads by cigar hard clipping 
                        if cigar[-1] == 'H':
                            hc_right = int(re.split("([A-Z])",cigar)[-3])
                            sam_seq = sam_seq[0 : -hc_right]
