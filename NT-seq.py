    ...
    for file in FILE:
        ...
        for col_file in collected_files:
            if not is_fastq(col_file):
                ...
                logging.warn("NTseq found '{}' file, but this seems not to be a FASTQ file.".format(col_file))
...
def run_NTseq_pipeline(param):
    import snakemake
    snakefile = os.path.join(os.path.dirname(__file__), "scripts/Snakefile.paired" if param["paired"] else "scripts/Snakefile.single")
...
def run(args):
    if args['quant']:
        ...
        if args['--outpath'] is None:
            args['--outpath'] = os.path.join(os.getcwd(), "NTseq_output/")
...
if __name__ == '__main__':
    ...
    args = docopt(__doc__, version='NT-seq v0.1')
    run(args)